...
from diffusers import AutoencoderOobleck, StableAudioDiTModel
...
class MusicTransferPipeline(DiffusionPipeline):
    def __init__(
        self,
    ...
    def __call__(
        ...
        ## ---------- SDEdit ---------- ## 
        input_audio, _ = torchaudio.load(audio_path, format='mp3')
        input_audio = input_audio.to(device)
        input_audio = input_audio.unsqueeze(0).to(torch.float16)
        audio_vae_length = int(self.transformer.config.sample_size * self.vae.hop_length)
        audio_shape = (batch_size // num_waveforms_per_prompt, self.vae.config.audio_channels, int(audio_vae_length))
        """
        # check num_channels
        if input_audio.shape[1] == 1 and self.vae.config.audio_channels == 2:
            input_audio = input_audio.repeat(1, 2, 1)
        elif input_audio.shape[1] == 2 and self.vae.config.audio_channels == 1:
            input_audio = input_audio.mean(1, keepdim=True)
        ...
        audio = input_audio.new_zeros(audio_shape)
        audio[:, :, : min(audio_length, audio_vae_length)] = input_audio[:, :, :audio_vae_length]
        """
        encoded_audio = self.vae.encode(input_audio).latent_dist.sample(generator)
        encoded_audio = encoded_audio.repeat((num_waveforms_per_prompt, 1, 1))
        noise = torch.randn_like(encoded_audio)
        # Decide the noise level
        shallow_reverse_step = int(num_inference_steps * (1 - noise_scale))
        timesteps = timesteps[shallow_reverse_step:]
        # print(timesteps)
        timesteps_tensor = torch.tensor([timesteps[0]], dtype=torch.float32)
        # Add the coresponding noise to the latent
        noisy_sample = self.scheduler.add_noise(encoded_audio, noise, timesteps_tensor)
