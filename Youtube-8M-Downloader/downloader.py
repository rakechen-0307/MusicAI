...
class Download():
    ...
    def download(self, categories_name = "Maltese"):
        # id_path = os.path.join(self.PATH, "ID", '{}.txt'.format(categories_name))
        # id_list = open(id_path, 'r+')
        # video_id_list = id_list.readlines()
        video_id_list = self.download_id(categories_name)
        if video_id_list != None:
            self.download_video(video_id_list[:len(video_id_list)//20], categories_name)
    ...
    def download_video(self, video_id_list, categories_name):
        ...
        video_dir= os.path.join(self.PATH,'videos',categories_name) 
        create_folder(video_dir)
        for video_id in video_id_list:
            os.system("yt-dlp --paths './videos/Music video' --output '%(title)s.%(ext)s' --extract-audio --audio-format mp3 'https://www.youtube.com/watch?v={}'".format(video_id))
